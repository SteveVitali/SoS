...
let notificationPoster: NotificationPoster | null = null;

export function setNotificationPoster(poster: NotificationPoster) {
  notificationPoster = poster;
}

/** @deprecated Use setNotificationPoster instead */
export function setSlackPoster(poster: NotificationPoster) {
  setNotificationPoster(poster);
}
...
export async function createJobFromSlack(
  ...
  try {
    ...
    // Post queued notification
    if (notificationPoster) {
      await notificationPoster.postQueued(job);
    }
...
// --- Create from Discord ---
export async function createJobFromDiscord(
  input: CreateJobFromDiscord,
): Promise<{ job: JobDoc; created: boolean }> {
  // Idempotency check
  const existing = await checkIdempotent(input.event_id);
  if (existing) {
    log.info("Duplicate Discord event, returning existing job", {
      event_id: input.event_id,
      task_id: existing.task_id,
    });
    return { job: existing, created: false };
  ...
  const doc: JobDoc = {
    task_id: taskId,
    source: { type: "discord_mention", event_id: input.event_id },
    requested_by: input.requested_by,
    discord_requester: input.discord_requester,
    status: "QUEUED",
    created_at: now,
    updated_at: now,
    discord: {
    ...
    ...(input.needs_plan ? { needs_plan: true } : {}),
    ...(input.custom_instructions ? { custom_instructions: input.custom_instructions } : {}),
    events: [{ at: now, type: "QUEUED", payload: { source: "discord" } }],
  };

  try {
    const job = await insertJob(doc);
    log.info("Job created from Discord", {
      task_id: taskId,
      event_id: input.event_id,
      attachments: input.attachments?.length || 0,
    });
...
export async function createRespondToCommentsJob(
  ...
  source: "web" | "slack" | "discord" = "web",
  slack?: { channel_id: string; thread_ts: string },
  discord?: { channel_id: string; thread_id?: string; guild_id?: string },
): Promise<JobDoc> {
  const now = nowDate();
  const taskId = uuidv4();
...
export async function claim(
  ...
  if (job) {
    if (notificationPoster) {
      ...
      if ((job.attempt || 1) <= 1) {
        await notificationPoster.postClaimed(job);
      }
    }
    if ((job.attempt || 1) <= 1) {
...
export async function handleWorkerEvent(
  ...
  if (SLACK_NOTIFY_EVENTS.includes(type as WorkerEventType) && notificationPoster && job) {
    await notificationPoster.postEvent(job, type, payload);
  }
...
export async function awaitApproval(
  ...
  if (job) {
    ...
    if (notificationPoster) {
      ...
      await notificationPoster.postEvent(job, "WAITING_FOR_APPROVAL", {
...
export async function promotePr(taskId: string, reviewers?: string[]) {
  ...
  if (job) {
    ...
    if (notificationPoster) {
      const prs = job.pr_urls?.length ? `\n${job.pr_urls.join(", ")}` : "";
      await notificationPoster.postEvent(job, "PR_PROMOTED", {
...
export async function complete(
  ...
  if (job) {
    ...
    if (notificationPoster) {
      await notificationPoster.postDone(job);
    }
...
export async function fail(
  ...
  if (job) {
    ...
    if (notificationPoster) {
      await notificationPoster.postFailed(job);
    }
...
export async function cancel(taskId: string) {
  ...
  if (job) {
    ...
    if (notificationPoster) {
      await notificationPoster.postCanceled(job);
    }
...
export async function submitPlan(
  ...
  if (job) {
    ...
    if (notificationPoster) {
      await notificationPoster.postPlan(job);
    }
