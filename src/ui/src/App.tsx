...
const css = {
  ...
  header: {
    ...
    padding: "16px 0", borderBottom: "1px solid var(--border)", marginBottom: 24,
  ...
  btn: {
    ...
    background: "var(--bg2)", color: "var(--fg)", fontSize: 14, fontWeight: 500,
  } as React.CSSProperties,
  btnPrimary: {
  ...
  btnSmall: {
    ...
    background: "var(--bg3)", color: "var(--fg2)", fontSize: 12,
  } as React.CSSProperties,
  card: {
  ...
  th: {
    textAlign: "left" as const, padding: "10px 12px", borderBottom: "1px solid var(--border)",
    color: "var(--fg2)", fontWeight: 600, fontSize: 12, textTransform: "uppercase" as const,
    letterSpacing: 0.5,
  },
  td: {
    padding: "10px 12px", borderBottom: "1px solid var(--border)", verticalAlign: "top" as const,
  },
  badge: (color: string) => ({
  ...
  input: {
    ...
    background: "var(--bg3)", color: "var(--fg)", fontSize: 14, width: "100%",
  } as React.CSSProperties,
  select: {
    padding: "8px 12px", borderRadius: "var(--radius)", border: "1px solid var(--border)",
    background: "var(--bg3)", color: "var(--fg)", fontSize: 14,
  } as React.CSSProperties,
  textarea: {
    padding: "8px 12px", borderRadius: "var(--radius)", border: "1px solid var(--border)",
    background: "var(--bg3)", color: "var(--fg)", fontSize: 14, width: "100%",
  ...
  pre: {
    background: "var(--bg)", padding: 12, borderRadius: "var(--radius)", fontSize: 12,
    fontFamily: "'SF Mono', Monaco, Consolas, monospace", overflowX: "auto" as const,
    border: "1px solid var(--border)", whiteSpace: "pre-wrap" as const, maxHeight: 300,
    overflow: "auto",
  } as React.CSSProperties,
  timeline: {
    borderLeft: "2px solid var(--border)", paddingLeft: 16, marginLeft: 8,
  } as React.CSSProperties,
  timelineItem: { marginBottom: 12, position: "relative" as const } as React.CSSProperties,
  dot: (color: string) => ({
    width: 10, height: 10, borderRadius: "50%", background: color,
    position: "absolute" as const, left: -22, top: 6,
  }),
  tokenSetup: {
    background: "var(--bg2)", padding: 24, borderRadius: "var(--radius)",
    border: "1px solid var(--border)", maxWidth: 400, margin: "80px auto",
  } as React.CSSProperties,
  ...
  return (
    <div style={css.tokenSetup}>
      ...
      <button
        ...
        onClick={() => { localStorage.setItem("sos_token", token); onSet(); }}
      >
        Connect
      </button>
    </div>
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [jobsRes, usersRes] = await Promise.all([
        listJobs({ status: status || undefined, requested_by: requestedBy || undefined, q: search || undefined, limit, offset }),
        getUsers(),
      ]);
    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        ...
        <div style={css.row}>
          ...
          <button style={css.btnPrimary} onClick={onCreateClick}>+ Create Job</button>
        </div>
      </div>
      <div style={css.filters}>
        <select style={css.select} value={status} onChange={(e) => { setStatus(e.target.value); setOffset(0); }}>
        ...
        <>
          <div style={{ overflowX: "auto" }}>
            <table style={css.table}>
              ...
              <tbody>
                {jobs.map((job) => (
                  <tr key={job.task_id} style={{ cursor: "pointer" }} onClick={() => onSelect(job.task_id)}>
                    ...
                    <td style={css.td} onClick={(e) => e.stopPropagation()}>
                      <div style={{ display: "flex", gap: 4 }}>
                        {["QUEUED", "RUNNING", "FIXING_CI"].includes(job.status) && (
                          <button style={css.btnSmall} onClick={() => handleAction("cancel", job.task_id)}>Cancel</button>
                        )}
                        {["FAILED", "CANCELED"].includes(job.status) && (
                          <button style={css.btnSmall} onClick={() => handleAction("retry", job.task_id)}>Retry</button>
                        )}
                        {!["RUNNING", "FIXING_CI"].includes(job.status) && (
                          <button style={{ ...css.btnSmall, color: "var(--red)" }} onClick={() => handleAction("delete", job.task_id)}>Del</button>
                        )}
                      </div>
                    </td>
                  </tr>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 12 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button style={css.btnSmall} disabled={offset === 0} onClick={() => setOffset(Math.max(0, offset - limit))}>
                ← Prev
              </button>
              <button style={css.btnSmall} disabled={offset + limit >= total} onClick={() => setOffset(offset + limit)}>
                Next →
              </button>
            </div>
          </div>
...
function JobDetail({
  ...
  return (
    <div>
      <button style={{ ...css.btn, marginBottom: 16 }} onClick={onBack}>← Back to Jobs</button>
      <div style={css.card}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
            ...
            <div style={css.row}>
              ...
              {job.parent_task_id && (
                <span style={{ fontSize: 13 }}>
                  Retry of{" "}
                  <span style={{ ...css.link, ...css.mono }} onClick={() => onNavigate(job.parent_task_id!)}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button style={css.btn} onClick={load}>↻</button>
            {["QUEUED", "RUNNING", "FIXING_CI"].includes(job.status) && (
              <button style={css.btnDanger} onClick={() => handleAction("cancel")}>Cancel</button>
            )}
            {["FAILED", "CANCELED"].includes(job.status) && (
              <button style={css.btnPrimary} onClick={() => handleAction("retry")}>Retry</button>
            )}
            {!["RUNNING", "FIXING_CI"].includes(job.status) && (
              <button style={css.btnDanger} onClick={() => handleAction("delete")}>Delete</button>
            )}
          </div>
        </div>
      ...
      <div style={css.card}>
        ...
        <div style={{ ...css.row, marginTop: 12, fontSize: 13, color: "var(--fg2)" }}>
          {job.repo_hint && <span>repo_hint: <b>{job.repo_hint}</b></span>}
          {job.test_level && <span>test_level: <b>{job.test_level}</b></span>}
          {job.ci_fix_enabled !== undefined && <span>ci_fix: <b>{job.ci_fix_enabled ? "on" : "off"}</b></span>}
          {job.reviewers?.length ? <span>reviewers: <b>{job.reviewers.join(", ")}</b></span> : null}
        </div>
      </div>
      ...
      {job.slack?.channel_id && (
        <div style={css.card}>
          ...
          <div style={{ fontSize: 13, color: "var(--fg2)" }}>
            Channel: <span style={css.mono}>{job.slack.channel_id}</span> ·
            Thread: <span style={css.mono}>{job.slack.thread_ts}</span>
            {job.slack.permalink && (
              <> · <a href={job.slack.permalink} target="_blank" rel="noopener">Open in Slack</a></>
            )}
          </div>
        </div>
      )}
      ...
      <div style={css.card}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 12, fontSize: 13 }}>
          ...
          {job.lease_expires_at && (
            <div><span style={{ color: "var(--fg2)" }}>Lease expires:</span> {relativeTime(job.lease_expires_at)}</div>
          )}
          {job.heartbeat_at && (
            <div><span style={{ color: "var(--fg2)" }}>Last heartbeat:</span> {relativeTime(job.heartbeat_at)}</div>
          )}
          {job.run_started_at && (
            <div><span style={{ color: "var(--fg2)" }}>Started:</span> {new Date(job.run_started_at).toLocaleString()}</div>
          )}
          {job.run_ended_at && (
            <div><span style={{ color: "var(--fg2)" }}>Ended:</span> {new Date(job.run_ended_at).toLocaleString()}</div>
          )}
        </div>
      </div>
      ...
      <div style={css.card}>
        ...
        {job.pr_urls?.length ? (
          <div style={{ marginBottom: 8 }}>
            ...
            {job.pr_urls.map((url, i) => (
              <a key={i} href={url} target="_blank" rel="noopener" style={{ marginRight: 12 }}>{url}</a>
            ))}
          </div>
        ) : null}
        {job.ci?.runs?.length ? (
          <div style={{ marginBottom: 8 }}>
            ...
            {job.ci.runs.map((run, i) => (
              <span key={i} style={{ marginRight: 12, fontSize: 13 }}>
                <a href={run.url} target="_blank" rel="noopener">{run.status}</a>
        ...
        {job.error && (
          <div style={{ marginTop: 8 }}>
            <span style={{ color: "var(--red)", fontSize: 13 }}>Error{job.error.code ? ` (${job.error.code})` : ""}:</span>
            <div style={{ ...css.pre, borderColor: "#ef444444" }}>{job.error.message}{job.error.details ? `\n\n${typeof job.error.details === 'string' ? job.error.details : JSON.stringify(job.error.details, null, 2)}` : ""}</div>
          </div>
        )}
      </div>
        ...
        <div style={css.card}>
          ...
          <div style={css.timeline}>
            {[...job.events].reverse().map((ev, i) => {
              const color = ev.type.includes("FAIL") || ev.type === "CANCELED"
                ? "var(--red)"
              ...
              return (
                <div key={i} style={css.timelineItem}>
                  ...
                  <div style={{ display: "flex", gap: 8, alignItems: "baseline" }}>
                    ...
                    {ev.node_id && <span style={{ ...css.mono, color: "var(--fg3)", fontSize: 11 }}>{ev.node_id}</span>}
                  </div>
                  {ev.payload && (
...
function CreateJobForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await createJob({
        ...
        reviewers: reviewers ? reviewers.split(",").map((r) => r.trim()).filter(Boolean) : undefined,
      });
  ...
  return (
    <div>
      <button style={{ ...css.btn, marginBottom: 16 }} onClick={onCancel}>← Back</button>
      <div style={css.card}>
        ...
        <form onSubmit={handleSubmit}>
          <div style={css.field}>
            ...
            <input style={css.input} value={requestedBy} onChange={(e) => setRequestedBy(e.target.value)} placeholder="U..." />
          </div>
          <div style={css.field}>
            <label style={css.label}>Task Text *</label>
            <textarea style={css.textarea} value={taskText} onChange={(e) => setTaskText(e.target.value)} placeholder="Describe the coding task..." />
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            <div style={css.field}>
              <label style={css.label}>Repo Hint (optional)</label>
              <input style={css.input} value={repoHint} onChange={(e) => setRepoHint(e.target.value)} placeholder="e.g. my-app" />
            </div>
            <div style={css.field}>
              <label style={css.label}>Test Level</label>
              <select style={{ ...css.select, width: "100%" }} value={testLevel} onChange={(e) => setTestLevel(e.target.value)}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            <div style={css.field}>
              ...
              <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer" }}>
                <input type="checkbox" checked={ciFixEnabled} onChange={(e) => setCiFixEnabled(e.target.checked)} />
                <span style={{ fontSize: 14, color: "var(--fg2)" }}>Enable CI fix attempts</span>
              </label>
            </div>
            <div style={css.field}>
              <label style={css.label}>Reviewers (comma-separated)</label>
              <input style={css.input} value={reviewers} onChange={(e) => setReviewers(e.target.value)} placeholder="alice, bob" />
            </div>
          </div>
          {error && <div style={css.error}>{error}</div>}
          <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
            ...
            <button type="button" style={css.btn} onClick={onCancel}>Cancel</button>
          </div>
        </form>
      </div>
    </div>
