...
- **Persistent memory system** — interaction-agnostic learning system that records episodes from all interaction paths (Slack, Discord, web chat), extracts facts via Mem0-style ADD/UPDATE/DELETE/NOOP curation, collects implicit feedback signals (gratitude, correction, rephrase, job outcomes), runs periodic reflection to consolidate episodes into higher-level insights and user profiles, and self-organizes memories via A-MEM-inspired link evolution. Five async pipelines: Episode Recording (zero LLM), Fact Extraction (1-2 LLM calls), Signal Collection (zero LLM), Reflection & Consolidation (N LLM calls), Memory Evolution (0-1 LLM calls). Retrieved memories injected into system prompt as `{MEMORY_CONTEXT}` and `{USER_CONTEXT}` using hybrid search (LanceDB vector + SQLite FTS5 keyword + RRF) with composite scoring (similarity + recency + importance + access frequency). Full HTTP API at `/api/web/memory/*` for browsing, searching, editing, and managing memories, episodes, and configuration. Three new MongoDB collections (`memories`, `interaction_episodes`, `memory_config`). Per-owner LanceDB tables and SQLite FTS5 indexes for memory embeddings. New `memory` model role in model config.
